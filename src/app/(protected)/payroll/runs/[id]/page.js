  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
        ...
        <Card><CardHeader><CardTitle>Processing control</CardTitle><CardDescription>Polling preserves the last known state during network errors.</CardDescription></CardHeader><CardContent className="flex flex-col gap-3"><RunPolling initialStatus={detail.run.status} initialDelayed={detail.delayed} initialRecoveryEligible={detail.recoveryEligible} runId={runId} />{detail.run.status === "queued" && detail.run.queueStatus !== "submitted" ? <form action={resubmitPayrollAction}><input name="runId" type="hidden" value={runId} /><Button type="submit"><SendIcon data-icon="inline-start" />Resubmit queued run</Button></form> : null}{detail.recoveryEligible ? <form action={recoverPayrollAction}><input name="runId" type="hidden" value={runId} /><Button type="submit" variant="destructive"><ShieldAlertIcon data-icon="inline-start" />Recover delayed run</Button></form> : null}{detail.run.status === "failed" ? <form action={retryPayrollAction}><input name="runId" type="hidden" value={runId} /><Button type="submit"><RotateCcwIcon data-icon="inline-start" />Retry frozen run</Button></form> : null}</CardContent></Card>
      </div>
      <section aria-labelledby="payouts-title"><div className="mb-4"><h2 className="text-xl font-semibold" id="payouts-title">Payouts and payslips</h2><p className="mt-1 text-sm text-muted-foreground">Finalized means calculated and locked, not externally transferred.</p></div><div className="flex flex-col gap-3">{detail.payouts.map(({ payout, payslip, earnings }) => { const overtime = earnings.find((earning) => earning.earningType === "overtime"); return <ResponsiveRecord action={payslip?.status === "generated" ? <PayslipDownload payslipId={payslip.id} /> : <Badge variant="outline">Payslip {payslip?.status ?? "pending"}</Badge>} key={payout.id} priorityValues={[{ label: "Employee number", value: payout.employeeNumber }, { label: "Net owed", value: formatPayrollMoney(payout.netAmountMinor, payout.currency, payout.currencyExponent) }]} secondaryValues={[{ label: "Base gross", value: formatPayrollMoney(payout.grossAmountMinor - (overtime?.amountMinor ?? 0), payout.currency, payout.currencyExponent) }, { label: "Overtime", value: overtime ? `${overtime.payableMinutes} min · ${formatPayrollMoney(overtime.amountMinor, payout.currency, payout.currencyExponent)}` : "None" }, { label: "Gross", value: formatPayrollMoney(payout.grossAmountMinor, payout.currency, payout.currencyExponent) }, { label: "Deductions", value: formatPayrollMoney(payout.deductionsAmountMinor, payout.currency, payout.currencyExponent) }]} title={payout.legalName} />; })}{detail.payoutNextCursor ? <Link className={buttonVariants({ variant: "outline" })} href={`/payroll/runs/${runId}?cursor=${encodeURIComponent(detail.payoutNextCursor)}`}>More payouts</Link> : null}</div></section>
